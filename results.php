...
$sex = $_POST['sex'];
// echo "sex: $sex\n";
$adv_where = $_POST['adv_where']; //figure out where this is coming from
//echo "adv_where: $adv_where\n";

if($_POST['reg_year'] != '-1' && $_POST['reg_month'] != '-1' && $_POST['reg_day'] != '-1') { 
    $reg_date = $_POST['reg_year'].'-'.$_POST['reg_month'].'-'.$_POST['reg_day']; 
} else { 
    $reg_date = '-1'; 
}
...
if (isset($_POST['vote_history_type'])) {
	...
	if ($vote_num > 0 && $vote_range > 0 && $vote_range >= $vote_num && in_array($vote_rangetype, array('years', 'elections'))) {
				...
				$thisyear = date('y', $data_age);	//prints the last two digits of a year. eg "15", "14"
				//echo $thisyear;
				// Dirty!! **TODO**
				...
				//echo strToTime('November 7, '. date('Y'));
	...
	foreach ($filter as $f) {
		...
		if (is_array($_POST[$f])) {
			...
			if($_POST[$f][0] == '-1') {
				...
				debug('<td>'. count($_POST[$f]) . (isset($_POST['exclude']) && is_array($_POST['exclude']) && in_array($f, $_POST['exclude']) ? ' - exclude' : '') .' Value - '.implode("','", $_POST[$f]).'</td></tr>', FALSE);
				$$all = "'". implode("','", $_POST[$f]) ."'" ;
			}
		}
	}
		...
		foreach ($_POST['cols'] as $c) {
			...
			switch ($c) {
				case 'phone':
				    ...
				    $adv_select[] = " concat(i.area_code, ' ', i.telephone) as phone ";
					break;

				case 'reg_source':
					$adv_select[] = "i.reg_soruce AS registration_source";
					break;

				case 'reg_datetime':
					$adv_select[] = "LEFT(i.{$c},10) AS registration_date";
					break;
				...
				case 'eye_color':
					$adv_select[] = "i.{$c} AS eye_color";
					break;

				case 'voter_status':
					$adv_select[] = "IF(STRCMP(i.{$c},'A'),'Inactive','Active') AS voter_status";	
					//$adv_select[] = "i.{$c} AS voter_status";
					break;

				case 'dob':
					$adv_select[] = "LEFT(i.{$c},10) AS DOB";
					break;
				...
				case 'reason':
					$adv_select[] = "i.{$c} AS voter_status_reason";
					break;
					
				case 'absentee':
					$adv_select[] = "i.{$c} AS absentee";
					$adv_select[] = "i.abs_application AS Absentee_Received_Date";
					break;
				// case 'town':
					 // $query = 'call get_filter(\'' . 'town' . '\', \'' . strToLower($county) . '\')';
					 // $result = $link->query($query);
					 // $count = mysqli_num_rows($result);
					 // $town = $_POST['town'];
					 
					// foreach($_POST['town'] as $j){
						// for($i=0; $i<$count; $i++) {
						// $row = mysqli_fetch_array($result);
						// if($j === $row[0]){
							// $town_name = $row[1];
							// echo $town_name;
							// $adv_select[] = $town_name." AS town";
							// continue;
						// }
					// }
					// }
	...
	switch ($todo) {
		...
		case 'count':
			...
			"$ward_all<br>",
			"$district_all<br>",
			"$cong_district_all<br>",
			"$sen_district_all<br>",
			"$leg_district_all<br>",
			"$asm_district_all<br>",
			"$school_district_all<br>",
			"$village_all<br>",
			"$fire_district_all<br>",
			"$user1_all<br>",
			"$user2_all<br>",
			"$user3_all<br>",
			"$user4_all<br>",
...
QRY;
			...
			//echo $result;		
			$row = mysqli_fetch_array($result);
			//echo("<script>console.log('Test count: ".count($result)."');</script>");
...
QRY;
			...
			$result = $link->query($query);
			$count = mysqli_num_rows($result);
			print("<h2>Raw count for selection grouped by last name: " . $count . " Voters</h2>");
			...
			for ($i=0; $i<$count; $i++) {
				...
				if ($household                                                         // we're householding, yes!
                    ...
                    // Combine parties on output
                    $prevParty = trim($prev['party']);
                    $curParty = trim($cur['party']);
                    // If we have multiple previous parties
                    if ($prevParty && $curParty)
                    {
                        $cur['party'] = "$prevParty, $curParty";
                    }
